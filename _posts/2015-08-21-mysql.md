...
### 目录
* [Mac如何卸载MySQL](#mac如何卸载mysql)
* [错误锦集](#错误锦集)
* [MySQL事务隔离级别](#mysql事务隔离级别)
* [MySQL返回自增长ID的四种方法](#mysql返回自增长id的四种方法)
* [获取表的所有字段名](#获取表的所有字段名)
...
```vim
SELECT COLUMN_NAME FROM information_schema. COLUMNS WHERE TABLE_SCHEMA = '数据库名' AND TABLE_NAME = '表名' ORDER BY COLUMN_NAME;
```
