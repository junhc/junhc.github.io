...
#### MySQL返回自增长ID的四种方法
```vim
1.SELECT MAX(_ID) FROM TABLE_NAME;	// 高并发下MAX(_ID)显然不能使用，这时可以使用LAST_INSERT_ID
...
2.SELECT LAST_INSERT_ID();		// LAST_INSERT_ID与TABLE无关的，如果向表A插入数据后，再向表B插入数据，LAST_INSERT_ID会改变。
3.SELECT @@IDENTITY;			// @@IDENTITY表示的是最近一次向具有IDENTITY属性(即自增列)的表插入数据时对应的自增列的值，是系统定义的全局变量。
...
4.SHOW TABLE STATUS;			// 得出的结果里边对应表名记录中有个AUTO_INCREMENT字段，里边有下一个自增ID的数值就是当前该表的最大自增ID
```
